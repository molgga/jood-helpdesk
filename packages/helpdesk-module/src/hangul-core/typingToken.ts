import { KR_SOURCE_FIRST, KR_WORD_START_AT, KR_WORD_END_AT, SIZE_SOURCE_LAST, SIZE_COMBINE_MEDIAL } from './types';
...
export function typingToken(sourceChar: string) {
  ...
  if (KR_WORD_START_AT <= codeAt && codeAt <= KR_WORD_END_AT) {
    const initialIndex = Math.floor((codeAt - KR_WORD_START_AT) / SIZE_COMBINE_MEDIAL);
    const initialAt = initialIndex * SIZE_COMBINE_MEDIAL + KR_WORD_START_AT;
    const medialIndex = Math.floor((codeAt - initialAt) / SIZE_SOURCE_LAST) * SIZE_SOURCE_LAST + initialAt;
    const medialAt = Math.floor((codeAt - initialAt) / SIZE_SOURCE_LAST);
    ...
    if (9 <= medialAt && medialAt <= 11) {
      addMedialCode = initialAt + 8 * SIZE_SOURCE_LAST; // ㅘ, ㅙ, ㅚ (9, 10, 11) -> add ㅗ (8)
    } else if (14 <= medialAt && medialAt <= 16) {
      addMedialCode = initialAt + 13 * SIZE_SOURCE_LAST; // ㅝ, ㅞ, ㅟ (14, 15, 16) -> add ㅜ (13)
    } else if (medialAt === 19) {
      addMedialCode = initialAt + 18 * SIZE_SOURCE_LAST; // ㅢ (19) -> add ㅡ (18)
    }
